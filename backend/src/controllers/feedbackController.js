...
// Inicializa a IA usando a chave do .env
  ...
  try {
    ...
    if (!comentarioBruto || comentarioBruto.trim() === '') {
      return res.status(400).json({
        erro: 'O comentário bruto é obrigatório.'
      });
    }
    ...
    const instrucaoPrompt = `
      ...
      Não use blocos de código com markdown (como \`\`\`json)
      e não coloque nenhum texto antes ou depois do objeto.

      O JSON deve conter exatamente estes 3 campos:

      1. "sentimento": Deve ser estritamente uma destas opções:
      "Positivo", "Negativo" ou "Neutro".

      2. "nota": Um número inteiro de 1 a 5
      (onde 1 é muito insatisfeito e 5 é muito satisfeito).
    ...
    console.error(
      '❌ Erro no processamento do feedback:',
      error
    );

    return res.status(500).json({
      erro: 'Erro interno ao processar a análise com IA.',
...
const listarFeedbacks = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      erro: 'Erro ao buscar o histórico de feedbacks.'
    });
  }
};
